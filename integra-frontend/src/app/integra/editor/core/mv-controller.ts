...
import { WiringController } from '../features/wiring/wiring-controller';
...
export class MVController {
  ...
  public mountFeature(feature: string) {
    if (this.isMounted) {
      switch (feature) {
        case 'import': {
          break;
        ...
        case MVC_STATE.SELECTION: {
          ...
          if (this.hover) {
            if (cellView.model.isElement()) {
              const props = DiagramHelper.getElementViewModelProp(cellView);
              if (props) {
                const cfg = DiagramHelper.getModelConfig(
                  this,
                  props.cfgName
                );
                if (cfg && cfg.type === 'diagram') {
                  if (cfg.hover.isHoverable) {
                    this.hover.elementHover(cellView, cfg, props, 'true');
                  }
                }
              }
    ...
    if ((this, this.isMounted)) {
      ...
      if (this.hover) {
        if (cellView.model.isElement()) {
          const prop = DiagramHelper.getElementViewModelProp(cellView);
          if (prop) {
            const cfg = DiagramHelper.getModelConfig(this, prop.cfgName);
            if (cfg && cfg.type === 'diagram') {
              if (cfg.hover.isHoverable) {
                this.hover.elementHover(cellView, cfg, prop, 'false');
              }
            }
          }
        } else if (cellView.model.isLink()) {
  ...
  public link_connect(
    ...
    const props = DiagramHelper.getLinkModelProp(view.model);
    if (props && props.model === 'wire') {
      const targetElem = this.paper.getModelById(view.model.target().id) as joint.dia.Element;
      if (targetElem) {
        const targertPort = targetElem.getPort(view.model.target().port);
        if (targertPort) {
          const targetPortGroup = targetElem.getPort(view.model.target().port).group;
          (targetElem as unknown as ModelInterface).setModelProp('lock', targetPortGroup);
        }
      }
    }
  }
    ...
    switch (this.state) {
      case MVC_STATE.SELECTION: {
        if (cell.isLink()) {
          ...
          const props = DiagramHelper.getLinkModelProp(link);
          if (props && props.model === 'wire') {
            const sourceElem = this.paper.getModelById(link.source().id) as joint.dia.Element;
            if (sourceElem) {
              const sourcePortGroup = sourceElem.getPort(link.source().port).group;
              (sourceElem as unknown as ModelInterface).setModelProp('unlock', sourcePortGroup);
            }
            const targetElem = this.paper.getModelById(link.target().id) as joint.dia.Element;
            if (targetElem) {
              const targetPortGroup = targetElem.getPort(link.target().port).group;
              (targetElem as unknown as ModelInterface).setModelProp('unlock', targetPortGroup);
            }
          }
