...
export class ZoomRubberBand {
  ...
  public drawRubberBand(start: IPoint, current: IPoint) {
    ...
    if (Math.abs(dx) > tol || Math.abs(dy) > tol) {
      if (this.rubberbandShape) {
        ...
        this.rubberbandShape.attr('visibility', true);
        this.rubberbandRect = new joint.g.Rect(x, y, width, height);
      }
    }
  }

  public deleteRubberBand() {
    if (this.rubberbandShape) {
