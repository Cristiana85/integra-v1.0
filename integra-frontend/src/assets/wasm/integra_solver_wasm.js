...
export class Session {
    ...
    run(run_id) {
        ...
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.session_run(this.__wbg_ptr, ptr0, len0);
        if (ret[1]) {
            throw takeFromExternrefTable0(ret[0]);
        }
    }
    ...
    get_last_dataset_envelope_json() {
        const ret = wasm.session_get_last_dataset_envelope_json(this.__wbg_ptr);
        let v1;
        if (ret[0] !== 0) {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('integra_solver_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
